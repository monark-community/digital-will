...
export async function validateSignUp(req: Request, res: Response, next: NextFunction) {
  ...
  if (!firstName || !lastName || !email || !password || !confirmPassword) {
    throw new BadRequestError('Missing required fields: firstName, lastName, email, password, confirmPassword');
  }
  ...
  if (password.length < PASSWORD.MIN_LENGTH) {
    throw new BadRequestError(`Password must be at least ${PASSWORD.MIN_LENGTH} characters long`);
  }
