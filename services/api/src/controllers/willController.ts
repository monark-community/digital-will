 ...
 */
export const handleGetWills = asyncHandler(
 ...
 */
export const handleGetAssociatedWills = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    const userId = req.user?.userId;
    if (!userId) {
      res
...
export const handleUpdateDraft = async (
  ...
  try {
    ...
    const { willName, secondaryMembers, minSecurityPeriod, maxSecurityPeriod } =
      req.body;
...
export const handleCancelWillOnChain = async (
  ...
  try {
    ...
    if (minSecurityPeriod === undefined)
      throw new BadRequestError("minSecurityPeriod is required");
    if (maxSecurityPeriod === undefined)
      throw new BadRequestError("maxSecurityPeriod is required");
    if (!secondaryMembersVotingPowers)
      throw new BadRequestError("secondaryMembersVotingPowers is required");
...
// keeping only the offchain information update
...
export const handleGetContractBalance = async (
  ...
  try {
    ...
    if (!contractAddress)
      throw new BadRequestError("Contract address is required");
 ...
 */
export const handleGetEnrichedWills = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    const { walletAddress } = req.params;
    const userId = req.user?.userId;
